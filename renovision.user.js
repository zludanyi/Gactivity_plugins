...
// @run-at       document-end
// ==/UserScript==
(async function RenovisionPackage() {
   'use strict';
   ...
   /* LOGGING CONFIG */
    ...
    (async function Renovision() {
        ...
        function createFeatureExtractionWorkerScriptContent() {
            logger.debug("Creating worker script content string for feature extraction v"+appVersion);
        ...
        async function promptForApiKey() {
            logger.debug("New Promise: Prompt user for API Key via modal v"+appVersion);
        ...
        async function getBuildingData(addressQueryString) {
            logger.debug("New Promise: getBuildingData v"+appVersion, addressQueryString);
            return new Promise((resolve, reject) => {
                if (!db) {
        ...
        async function clearIndexedDB() {
            logger.info("clearIndexedDB v"+appVersion+" is called.");
            if (db) {
            ...
            logger.debug("New Promise: Delete IndexedDB v"+appVersion, DB_NAME);
            return new Promise((resolve, reject) => {
        ...
        async function getVerifiedFeatures(limit = MAX_EMBEDDINGS_FOR_KNN_QUERY) {
            logger.debug("New Promise: getVerifiedFeatures v"+appVersion+", limit:", limit);
            return new Promise((resolve, reject) => {
                if (!db) {
        ...
        function injectNativeCSS() {
            logger.debug("Injecting native CSS v"+appVersion);
            const css = `
                .renovision-toggle-button-zl { /* Changed class name slightly for uniqueness */
                    position: fixed;
                    bottom: 75px; /* Adjust to be above your status bar */
                    right: 15px;
                    ...
                    line-height: 50px; /* Vertically center icon text */
                    text-align: center; /* Horizontally center icon text */
                    cursor: pointer;
                    box-shadow: 0 2px 8px rgba(0,0,0,0.4);
                    z-index: 2147483641; /* Higher than the main panel, but below dialogs */
                    display: flex; /* For centering if using SVG/img */
                    justify-content: center;
                    align-items: center;
                    transition: background-color 0.3s ease, transform 0.2s ease;
                }
                .renovision-toggle-button-zl:hover {
                    background-color: #0056b3;
                    transform: scale(1.1);
                }
                .renovision-main-container-zl.hidden-zl { /* Class to hide the main panel */
                    display: none !important;
                }
                /* Main Panel Container */
                #tm-renovision-panel-zl-v19final {
                    position: fixed;
                    top: 10px;
                    left: 10px;
                    width: 650px;
                    ...
                    border-radius: 8px;
                    padding: 15px;
                    z-index: 2147483640; /* Very high, but below critical browser UI elements */
                    overflow-y: auto;
                ...
                /* Headings within the Panel */
                #tm-renovision-panel-zl-v19final h2,
                #tm-renovision-panel-zl-v19final h3 {
                    margin-top: 0;
                    color: #007bff;
                    font-size: 18px;
                }
                ...
                #tm-renovision-panel-zl-v19final button {
                    background-color: #007bff;
                    color: white;
                    border: none;
                    padding: 8px 12px;
                ...
                #tm-renovision-panel-zl-v19final button:hover {
                    background-color: #0056b3;
                }

                #tm-renovision-panel-zl-v19final button:disabled {
                    background-color: #ccc;
                    cursor: not-allowed;
                }

                /* Google Maps Div */
                #tm-gmaps-map-renovision-zl-v19final {
                    width: 100%;
                    height: 200px;
                    margin: 10px 0;
                ...
                .results-table-zl-v19final th,
                .results-table-zl-v19final td {
                ...
                /* Individual Result Row (for hover effects) */
                .result-row-zl-v19final {
                    cursor: default; /* Changed from pointer to allow overlay interaction */
                    position: relative; /* For absolute positioning of switch overlay */
                }

                .result-row-zl-v19final:hover .switch-status-overlay-zl-v19final {
                    opacity: 1;
                    visibility: visible;
                }

                /* Log Panel and Uncertain Images List */
                #tm-log-renovision-zl-v19final,
                #tm-uncertain-images-zl-v19final-list {
                    margin-top: 10px;
                    padding: 8px;
                ...
                /* Summary Div */
                #tm-summary-renovision-zl-v19final {
                    margin-top: 10px;
                    padding: 8px;
                    background-color: #e9ecef;
                    border-radius: 4px;
                }
                /* Dialog Overlay (for API Key, Confirmation, Swipe Modals) */
                .dialog-overlay-zl-ui-v19final {
                    position: fixed;
                    top: 0;
                    left: 0;
                    width: 100%;
                    ...
                    justify-content: center;
                    align-items: center;
                    z-index: 2147483645; /* Highest z-index for modals */
                }
                /* Dialog Box (General Styling) */
                .dialog-box-zl-ui-v19final {
                    ...
                    min-width: 300px;
                    max-width: 90vw;
                    z-index: 2147483646; /* Above its own overlay */
                }
                .dialog-box-zl-ui-v19final p {
                    margin-bottom: 15px;
                    white-space: pre-wrap;
                    text-align: left;
                }
                .dialog-box-zl-ui-v19final button {
                    margin: 5px;
                }
                /* Bottom Status Bar */
                #tm-statusbar-zl-v19final { /* Assuming this is the ID for your status bar */
                    position: fixed;
                    bottom: 0;
                    left: 0;
                    width: 100%;
                    ...
                    color: #fff;
                    padding: 8px 15px;
                    z-index: 2147483630; /* High, but below main panel and toggle button */
                    font-size: 12px;
                    display: flex;
                ...
                /* Switch Status Overlay (on image hover in results) */
                .switch-status-overlay-zl-v19final {
                    position: absolute;
                    top: 0;
                    left: 0;
                    width: 100%;
                ...
                .tm-progress-bar-container-zl-v19final {
                    width: 200px;
                    height: 12px;
                ...
                #tm-progress-bar-rect-zl-v19final {
                    transition: width 0.3s ease;
                    /* fill is set in JS or directly on element */
                }

                /* ETR Text in Status Bar */
                #tm-etr-status-zl-v19final {
                    margin-left: 15px;
                    font-style: italic;
                    color: #ccc;
                }

                /* Batch Feedback Buttons Area */
                .batch-feedback-controls-zl-v19final button {
                    font-size: 11px;
                    padding: 4px 8px;
                    margin-left: 5px;
                }

                /* Swipe Modal (for Reference Set Definition) */
                .swipe-modal-zl-v19final .swipe-image-container { /* Specific to the swipe modal's image container */
                    width: 320px;
                    height: 240px;
                    margin: 10px auto;
                    border: 1px solid #ccc;
                    ...
                    display: flex; /* Added for centering text if no image */
                ...
                .swipe-modal-zl-v19final .swipe-actions button {
                    font-size: 18px; /* Larger buttons for easier clicking/tapping */
                    padding: 10px 15px;
                }

                /* Uncertain Item Styling in Results */
                .uncertain-item-zl-v19final {
                    background-color: #fff3cd; /* A light yellow, similar to Bootstrap's warning */
                    border-left: 3px solid #ffeeba !important; /* Emphasize with a border */
                }

                /* Image container for date strip positioning */
                .image-container-zl-v19final {
                    position: relative;
                    display: inline-block; /* Or block if you want it to take full width of li part */
                }

                /* Date strip below images in results */
                .date-strip-zl-v19final {
                    position: absolute;
                    bottom: 3px; /* Assumes img has margin-bottom:3px or adjust as needed */
                    left: 0;
                    width: 100%;
            ...
            logger.info("Native CSS styles injected v"+appVersion);
        }

        /**
         * Toggles the visibility of the main Renovision control panel.
         */
        function toggleAppVisibility() {
            logger.debug("Toggling app visibility.");
            if (controlPanel) {
                const isHidden = controlPanel.classList.toggle('hidden-zl');
                if (appToggleButton) { // Update button text/icon if needed
                    appToggleButton.innerHTML = isHidden ? '🏠' : '🙈'; // Example icons
                    appToggleButton.title = isHidden ? "Show Renovision Panel" : "Hide Renovision Panel";
                }
                logger.info("Renovision panel visibility toggled to:", isHidden ? "Hidden" : "Visible");
            } else {
                logger.warn("Control panel element not found, cannot toggle visibility.");
            }
        }
        ...
        function createUI() {
            ...
            appToggleButton = document.createElement('button');
            appToggleButton.id = 'renovision-app-toggle-button-zl';
            appToggleButton.className = 'renovision-toggle-button-zl';
            appToggleButton.innerHTML = '🏠'; // Or some other icon like R, SV
            appToggleButton.title = "Toggle Renovision Panel";
            appToggleButton.addEventListener('click', toggleAppVisibility);
            document.body.appendChild(appToggleButton);

            controlPanel = document.createElement('div');
            controlPanel.id = 'tm-renovision-panel-zl-v19final'; // Your main panel's ID
            controlPanel.className = 'renovision-main-container-zl hidden-zl'; // Add classes, START HIDDEN
            document.body.appendChild(controlPanel);
            controlPanel.innerHTML = `
                <h2>Renovision ver.${appVersion}</h2>
            ...
            logger.info("UI Created v"+appVersion);
        }

        function createBottomStatusBar() {
            logger.debug("Creating bottom status bar v"+appVersion);
            ...
            logger.info("Bottom status bar created v"+appVersion);
        ...
        async function handleStopCrawling() {
            ...
            logger.debug("New Promise: Stop Crawl Confirm Dialog v"+appVersion);
        ...
        /**
         * Converts an image URL to an ImageData-like object {data, width, height}.
         * It first checks an IndexedDB cache for the image blob. If not found,
         * it fetches the image from the URL, caches the blob, and then processes it.
         *
         * @async
         * @param {string} url - The URL of the image to process.
         * @returns {Promise<object|null>} A promise that resolves to an object
         *          {data: Uint8ClampedArray, width: number, height: number} if successful,
         *          or null if any step fails (e.g., fetch error, image load error).
         */
        async function urlToImageData(url) {
            logger.debug("New Promise: urlToImageData (with cache logic) for URL:", url ? url.slice(-50) : "null URL");

            if (!url) {
                logger.warn("urlToImageData: Received null or empty URL.");
                return null;
            }

            // Helper function to process a blob into an ImageData-like object
            const processBlobToImageData = (blob, sourceDescription) => {
                logger.debug("New Promise (inner): processBlobToImageData from", sourceDescription, "for URL:", url.slice(-30));
                return new Promise((resolve) => { // Changed to not use reject for simpler error chaining
                    const img = new Image();
                    let objectURL = ''; // Keep track to revoke it

                    img.onload = () => {
                        logger.debug("Image loaded into Image element from", sourceDescription, "for URL:", url.slice(-30), "Natural dims:", img.naturalWidth, "x", img.naturalHeight);
                        const canvas = document.createElement('canvas');
                        canvas.width = img.naturalWidth;
                        canvas.height = img.naturalHeight;

                        if (img.naturalWidth === 0 || img.naturalHeight === 0) {
                            logger.warn("Image (from " + sourceDescription + ") loaded with zero dimensions:", url);
                        ...
                        try {
                            const imageDataInstance = ctx.getImageData(0, 0, img.naturalWidth, img.naturalHeight);
                            logger.debug("ImageData created from", sourceDescription, "for URL", url.slice(-30));
                            if (objectURL) URL.revokeObjectURL(objectURL);
                            resolve({ data: imageDataInstance.data, width: imageDataInstance.width, height: imageDataInstance.height });
                        } catch (e) {
                            logger.error("Error getting ImageData from canvas (from " + sourceDescription + ", tainted canvas possible?):", e, url);
                            if (objectURL) URL.revokeObjectURL(objectURL);
                            resolve(null);
                        }
                    };
                    ...
                    img.onerror = (errEvent) => {
                        logger.error("Failed to load image from blob objectURL (from " + sourceDescription + "):", url, errEvent);
                        if (objectURL) URL.revokeObjectURL(objectURL);
                        resolve(null);
                    };

                    try {
                        objectURL = URL.createObjectURL(blob); // Create object URL from blob
                        img.src = objectURL; // Start loading the image from the object URL
                    } catch (e) {
                        logger.error("Error creating object URL or setting img.src (from " + sourceDescription + "):", e, url);
                        if (objectURL) URL.revokeObjectURL(objectURL); // Clean up if src setting fails
                        resolve(null);
                    }
                });
            };
            ...
            logger.debug("New Promise (implicit): getImageBlobFromCache in urlToImageData for URL:", url.slice(-50));
            const cachedBlob = await getImageBlobFromCache(url); // Assumes getImageBlobFromCache is defined

            if (cachedBlob) {
                logger.info("Image blob cache HIT for:", url.slice(-50));
                return processBlobToImageData(cachedBlob, "cache");
            }

            // 2. If not in cache, fetch from network
            logger.info("Image blob cache MISS for:", url.slice(-50), "Fetching from network.");
            try {
                logger.debug("New Promise (implicit): Fetching image from network for URL:", url.slice(-50));
                const response = await fetch(url); // Fetch the image URL
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status} for ${url}`);
                }
                logger.debug("New Promise (implicit): Converting network response to blob for URL:", url.slice(-50));
                const networkBlob = await response.blob(); // Get the response as a Blob

                // Asynchronously save to cache, don't wait for it to complete to speed up return
                saveImageBlobToCache(url, networkBlob).catch(e => { // Assumes saveImageBlobToCache is defined
                    logger.warn("Background image cache save failed for URL:", url.slice(-50), e);
                });

                return processBlobToImageData(networkBlob, "network");
            } catch (err) {
                logger.error("Failed to fetch or process image from network for URL:", url, err);
                return null; // Return null on any fetch/processing error
            }
        }

        /**
         * Reverse geocodes a google.maps.LatLng object to a human-readable address and extracts the house number.
         * Uses the Google Maps JavaScript API's Geocoder.
         *
         * @async
         * @param {google.maps.LatLng} latLng - The `google.maps.LatLng` object to reverse geocode.
         * @returns {Promise<object>} A promise that resolves to an object:
         *          { fullAddress: string, houseNumber: string | null }.
         *          `fullAddress` will be "N/A" and `houseNumber` null if reverse geocoding fails or no house number is found.
         */
        async function reverseGeocodeLatLng(latLng) {
            logger.debug("New Promise: Reverse Geocode LatLng", latLng.toJSON());

            if (!geocoder) {
                logger.error("Geocoder is not initialized. Cannot reverse geocode.");
                return Promise.resolve({ fullAddress: "N/A - Geocoder Error", houseNumber: null });
            }
            if (!(latLng instanceof google.maps.LatLng)) {
                logger.error("Invalid LatLng object provided to reverseGeocodeLatLng:", latLng);
                return Promise.resolve({ fullAddress: "N/A - Invalid Input", houseNumber: null });
            }

            return new Promise((resolve) => {
                logger.debug("Calling geocoder.geocode (reverse) for LatLng:", latLng.toJSON());
                geocoder.geocode({ 'location': latLng }, (results, status) => {
                    if (status === google.maps.GeocoderStatus.OK) {
                        if (results && results[0]) {
                            const address = results[0].formatted_address;
                            let houseNumber = null;
                            const streetNumberComponent = results[0].address_components.find(
                                c => c.types.includes('street_number')
                            );
                            if (streetNumberComponent) {
                                houseNumber = streetNumberComponent.long_name;
                            }
                            logger.info(
                                `Reverse geocoded OK to: "${address}" (HN: ${houseNumber || 'N/A'})`
                            );
                            resolve({ fullAddress: address, houseNumber: houseNumber });
                        } else {
                            logger.warn(
                                `Reverse geocoding for LatLng: ${latLng.toJSON()} returned OK but no valid results.`,
                                results
                            );
                            resolve({ fullAddress: "N/A - No Results", houseNumber: null });
                        }
                    } else {
                        logger.warn(
                            `Reverse geocoding failed for LatLng: ${latLng.toJSON()}. Status: ${status}`
                        );
                        // Optionally update status bar for these errors if frequent or critical
                        // updateBottomStatusBar({ ..., processStatus: `Error - Reverse Geocode Failed (${status})` });
                        resolve({ fullAddress: "N/A - Geocoding Failed", houseNumber: null });
                    }
                });
            });
        }
        ...
        /**
         * Gets the overview path (a series of LatLng points) for a route between an origin and destination.
         * Uses the Google Maps JavaScript API's DirectionsService.
         *
         * @async
         * @param {google.maps.LatLng} originLatLng - The starting LatLng for the directions request.
         * @param {google.maps.LatLng} destinationLatLng - The ending LatLng for the directions request.
         * @returns {Promise<Array<google.maps.LatLng>|null>} A promise that resolves to an array of `google.maps.LatLng`
         *                                                    objects representing the path if successful, otherwise null.
         */
        async function getDirectionsPath(originLatLng, destinationLatLng) {
            logger.debug(
                "New Promise: Get Directions Path from:",
                originLatLng.toJSON(),
                "to:",
                destinationLatLng.toJSON()
            );

            if (!directionsService) {
                logger.error("DirectionsService is not initialized. Cannot get directions path.");
                return Promise.resolve(null); // Or reject(new Error("DirectionsService not initialized"))
            }
            ...
            return new Promise((resolve) => {
                const request = {
                    ...
                    travelMode: google.maps.TravelMode.DRIVING // Or WALKING, depending on your needs
                };

                logger.debug("Calling directionsService.route with request:", request);
                directionsService.route(request, (result, status) => {
                    if (status === google.maps.DirectionsStatus.OK) {
                        if (result.routes && result.routes.length > 0 && result.routes[0].overview_path) {
                            logger.info(
                                "Directions path obtained successfully. Points:",
                                result.routes[0].overview_path.length
                            );
                            resolve(result.routes[0].overview_path);
                        } else {
                            logger.warn("DirectionsService returned OK but no routes or overview_path found.", result);
                            resolve(null);
                        }
                    } else {
                        logger.warn(
                            `Directions request failed. Status: ${status}. Origin:`, originLatLng.toJSON(), "Dest:", destinationLatLng.toJSON()
                        );
                        updateBottomStatusBar({ // Example of updating status on a specific API error
                            apiKey: API_KEY,
                            apiKeyFullStatus: API_KEY ? (localStorage.getItem(LOCALSTORAGE_API_KEY) ? "Registered (localStorage)" : "Session Only") : "Unregistered",
                            processStatus: `Error - Directions Failed (${status})`,
                            progress: null, etrString: ""
                        });
                        resolve(null);
                    }
                });
        ...
        /**
         * Geocodes a string address to a google.maps.LatLng object.
         * Uses the Google Maps JavaScript API's Geocoder.
         *
         * @async
         * @param {string} address - The string address to geocode.
         * @returns {Promise<google.maps.LatLng|null>} A promise that resolves to a `google.maps.LatLng` object
         *                                             if geocoding is successful, otherwise null.
         */
        async function geocodeAddress(address) {
            logger.debug("New Promise: Geocoding address", address);

            if (!geocoder) {
                logger.error("Geocoder is not initialized. Cannot geocode address.");
                return Promise.resolve(null); // Or reject(new Error("Geocoder not initialized"))
            }
            if (!address || typeof address !== 'string' || address.trim() === '') {
                logger.warn("geocodeAddress called with invalid address:", address);
                return Promise.resolve(null);
            }

            return new Promise((resolve) => {
                logger.debug("Calling geocoder.geocode for address:", address);
                geocoder.geocode({ 'address': address }, (results, status) => {
                    if (status === google.maps.GeocoderStatus.OK) {
                        if (results && results[0] && results[0].geometry && results[0].geometry.location) {
                            logger.info(
                                `Geocoded "${address}" successfully to: Lat: ${results[0].geometry.location.lat().toFixed(5)}, Lng: ${results[0].geometry.location.lng().toFixed(5)}`
                            );
                            resolve(results[0].geometry.location); // This is a google.maps.LatLng object
                        } else {
                            logger.warn(`Geocoding for "${address}" returned OK but no valid results.`, results);
                            resolve(null);
                        }
                    } else {
                        logger.warn(
                        ...
                        // Optionally update status bar for geocoding errors if they are critical to a flow
                        // updateBottomStatusBar({ ..., processStatus: `Error - Geocode Failed (${address.slice(0,15)}...: ${status})` });
                        resolve(null);
                    }
                });
            });
        ...
        /**
         * Fetches Street View panorama data for a given location, including the image capture date.
         * It uses the Google Maps JavaScript API's StreetViewService.
         *
         * @async
         * @param {google.maps.LatLng} locationForPanoSearch - The latitude and longitude around which to search for a panorama.
         * @param {number | null} [targetBuildingHeading=null] - Optional: The desired heading (0-360 degrees) for the Street View camera to face.
         *                                                      If null, the heading will be calculated from the Street View car's position towards `locationForPanoSearch`.
         * @param {number} [radius=30] - The radius, in meters, within which to search for the nearest panorama to `locationForPanoSearch`.
         * @returns {Promise<object|null>} A promise that resolves to an object containing Street View data if a panorama is found, otherwise null.
         * The resolved object has the following properties:
         * @property {string} imageUrl - The URL for the Street View static image, constructed with the obtained panorama ID and heading.
         * @property {string} panoId - The unique identifier for the Street View panorama.
         * @property {google.maps.LatLng} carLocation - The actual `google.maps.LatLng` object representing the Street View car's position for this panorama.
         * @property {number} heading - The compass heading (0-360) used for the `imageUrl`, pointing the camera.
         * @property {string} imageCaptureDate - The month and year the panorama was captured, in "YYYY-MM" format (e.g., "2023-05").
         */
        async function getStreetViewDataWithDate(locationForPanoSearch, targetBuildingHeading = null, radius = 30) {
            logger.debug(
                "New Promise: Get StreetView Data with Date. SearchLoc:",
                locationForPanoSearch.toJSON(),
                "TargetHeading:", targetBuildingHeading,
                "Radius:", radius
            );

            if (!API_KEY) {
                logger.error("API_KEY not set. Cannot make Street View Static API calls indirectly.");
                return Promise.resolve(null);
            }

            if (!streetViewService) {
                logger.error("StreetViewService is not initialized. Cannot get panorama data.");
                return Promise.resolve(null);
            }

            return new Promise((resolve) => {
                logger.debug(
                    "Calling streetViewService.getPanorama for location:",
                    locationForPanoSearch.toJSON(),
                    "Radius:", radius
                );
                ...
                streetViewService.getPanorama(
                    {
                        location: locationForPanoSearch,
                        radius: radius,
                    ...
                    (panoData, status) => {
                        if (status === google.maps.StreetViewStatus.OK) {
                            logger.debug("StreetViewService.getPanorama successful. Pano ID:", panoData.location.pano);

                            const panoId = panoData.location.pano;
                            const carLocationInstance = panoData.location.latLng;
                            const imageDate = panoData.imageDate; // "YYYY-MM"

                            const finalHeading = targetBuildingHeading !== null ?
                                targetBuildingHeading :
                                google.maps.geometry.spherical.computeHeading(carLocationInstance, locationForPanoSearch);

                            const imageUrl = `https://maps.googleapis.com/maps/api/streetview?size=${STREET_VIEW_IMAGE_REQUEST_SIZE}&pano=${panoId}&heading=${finalHeading}&pitch=${STREET_VIEW_PITCH}&fov=${STREET_VIEW_FOV}&key=${API_KEY}`;

                            logger.info(
                                `SV Data Retrieved: PanoID=${panoId}, Date=${imageDate}, FinalHeading=${finalHeading.toFixed(1)}`
                            );

                            resolve({
                                imageUrl: imageUrl,
                                panoId: panoId,
                                carLocation: carLocationInstance,
                                heading: finalHeading,
                                imageCaptureDate: imageDate
                            });
                        } else {
                            logger.warn(
                                `Street View panorama not found near Lat: ${locationForPanoSearch.lat().toFixed(5)}, Lng: ${locationForPanoSearch.lng().toFixed(5)}. Status: ${status}`
                            );
                            resolve(null);
                        }
                    }
        ...
        /**
         * Fetches metadata and image data (including Street View capture date) for a specific address.
         * Handles geocoding, Street View panorama search, and image data retrieval (potentially using blob cache via urlToImageData).
         * @async
         * @param {string} addressQueryString - The address to get image data for.
         * @param {string} [idForLog=''] - An identifier for logging purposes (e.g., 'base_0', 'ref_cand_X').
         * @returns {Promise<object|null>} A promise resolving to an object with image metadata (including imageCaptureDate)
         *                                 and the imageData object {data, width, height}, or null if any critical step fails.
         *                                 The returned object will also have an `error` property if a failure occurred.
         */
        async function getImageDataForAddress(addressQueryString, idForLog = '') {
            if (!isCrawlingGlobal && !idForLog.startsWith('base') && !idForLog.startsWith('ref_cand_modal')) {
                alert("hhhh");
                logger.info("getImageDataForAddress: Stop requested by isCrawlingGlobal flag for ID:", idForLog);
                return Promise.resolve({ addressQueryString, error: "Crawling stopped" });
            }
            alert("addressQueryString: "+addressQueryString);
            logger.debug(`New Promise: getImageDataForAddress v1.9.2 for ${idForLog}: ${addressQueryString}`);

            logger.debug(`Geocoding for ${idForLog}: ${addressQueryString}`);
            const location = await geocodeAddress(addressQueryString);
            if (!location) {
                logger.warn("Geocoding failed for", addressQueryString, "in getImageDataForAddress");
                return { addressQueryString, error: "Geocoding failed" };
            }

            logger.debug("New Promise (implicit): getStreetViewDataWithDate in getImageDataForAddress for", addressQueryString);
            const svResult = await getStreetViewDataWithDate(location, null, idForLog.startsWith('base') || idForLog.startsWith('ref_cand_modal') ? 100 : 30);

            if (!svResult || !svResult.imageUrl) {
                logger.warn("StreetView metadata fetch failed for", addressQueryString, "in getImageDataForAddress");
                const apiKeyStatusText = API_KEY ? (localStorage.getItem(LOCALSTORAGE_API_KEY) ? "Registered (localStorage)" : "Session Only") : "Unregistered";
                updateBottomStatusBar({
                    apiKey: API_KEY,
                    apiKeyFullStatus: apiKeyStatusText,
                    processStatus: `Error - SV Meta Fetch: ${addressQueryString.split(',')[0]}`,
                    progress: null, etrString: ""
                });
                if (idForLog.startsWith('base')) {
                    isCrawlingGlobal = false;
                }
                return { addressQueryString, loc: location.toJSON(), error: "StreetView metadata fetch failed" };
            }

            logger.debug("New Promise (implicit): urlToImageData in getImageDataForAddress for URL:", svResult.imageUrl.slice(-30));
            const imageDataObject = await urlToImageData(svResult.imageUrl);
            alert("current image: "+imageDataObject);
            if (!imageDataObject) {
                logger.warn("Failed to convert URL to ImageData for", addressQueryString, "in getImageDataForAddress");
                const apiKeyStatusText = API_KEY ? (localStorage.getItem(LOCALSTORAGE_API_KEY) ? "Registered (localStorage)" : "Session Only") : "Unregistered";
                updateBottomStatusBar({
                    apiKey: API_KEY,
                    apiKeyFullStatus: apiKeyStatusText,
                    processStatus: `Error - Image Data Load: ${addressQueryString.split(',')[0]}`,
                    progress: null, etrString: ""
                });
                if (idForLog.startsWith('base')) {
                    isCrawlingGlobal = false;
                }
                return { addressQueryString, loc: location.toJSON(), streetViewImageUrl: svResult.imageUrl, error: "Image data load failed" };
            }

            const result = {
                addressQueryString: addressQueryString,
                loc: location.toJSON(),
                streetViewImageUrl: svResult.imageUrl,
                streetViewPanoId: svResult.panoId,
                streetViewHeading: svResult.heading,
                imageCaptureDate: svResult.imageCaptureDate, // Capture date is included
                imageData: imageDataObject
            };
            logger.debug("Successfully got image data for", addressQueryString, "Date:", svResult.imageCaptureDate);
            return result;
        }

        /**
         * Fetches Street View image metadata (including capture date and imageData object) for multiple locations along a route.
         * This function handles geocoding the start/end points, getting directions, sampling points along the path,
         * and fetching Street View data for both left and right sides of the street at each sample point.
         * It respects the `isCrawlingGlobal` flag to allow interruption.
         * @async
         * @param {string} startAddress - The starting address of the street section.
         * @param {string} endAddress - The ending address of the street section.
         * @returns {Promise<Array<object>>} A promise that resolves to an array of objects. Each object represents a fetched image
         *                                 and its metadata (addressQueryString, houseNumber, loc, streetViewImageUrl, streetViewPanoId,
         *                                 streetViewHeading, carLocation, imageCaptureDate, imageData). Resolves to an empty array on critical failure.
         */
        async function fetchImagesForStreetSection(startAddress, endAddress) {
            if (!isCrawlingGlobal) {
                logger.info("fetchImagesForStreetSection: Stop requested pre-loop.");
                return [];
            }
            logger.info(`New Promise: fetchImagesForStreetSection v1.9.2 from "${startAddress}" to "${endAddress}"`);
            const collectedForStreet = [];

            logger.debug("New Promise (implicit): Geocoding start address in fetchImagesForStreetSection");
            const startLoc = await geocodeAddress(startAddress);
            logger.debug("New Promise (implicit): Geocoding end address in fetchImagesForStreetSection");
            const endLoc = await geocodeAddress(endAddress);

            const apiKeyStatusText = API_KEY ? (localStorage.getItem(LOCALSTORAGE_API_KEY) ? "Registered (localStorage)" : "Session Only") : "Unregistered";

            if (!startLoc || !endLoc) {
                logger.error("Could not geocode street section endpoints for fetchImagesForStreetSection.");
                updateBottomStatusBar({apiKey:API_KEY, apiKeyFullStatus: apiKeyStatusText, processStatus:"Error - Geocode Section Endpoints", progress:null, etrString:""});
                isCrawlingGlobal = false;
                return collectedForStreet;
            }
            logger.debug("Section endpoints geocoded for fetchImagesForStreetSection:", startLoc.toJSON(), endLoc.toJSON());

            logger.debug("New Promise (implicit): Getting directions path in fetchImagesForStreetSection");
            const path = await getDirectionsPath(startLoc, endLoc);

            if (!path || path.length < 2) {
                logger.warn("Could not get directions path for section in fetchImagesForStreetSection.");
                updateBottomStatusBar({apiKey:API_KEY, apiKeyFullStatus: apiKeyStatusText, processStatus:"Error - Directions Path Failed", progress:null, etrString:""});
                isCrawlingGlobal = false;
                return collectedForStreet;
            }
            logger.debug(`Directions path received with ${path.length} points for fetchImagesForStreetSection.`);

            let cumulativeDist = SAMPLING_INTERVAL_METERS;
            const processedPanoHeadings = new Set();

            for (let i = 0; i < path.length - 1; i++) {
                if (!isCrawlingGlobal) {
                    logger.info("fetchImagesForStreetSection: Loop interrupted by stop request at segment", i);
                    break;
                }
                const p1 = path[i];
                const p2 = path[i+1];
                const segDist = google.maps.geometry.spherical.computeDistanceBetween(p1,p2);
                let distAlongSeg = 0;

                while(distAlongSeg < segDist) {
                    if (!isCrawlingGlobal) {
                        logger.info("fetchImagesForStreetSection: Inner loop interrupted by stop request at segment", i, "distAlongSeg", distAlongSeg);
                        break;
                    }
                    if (cumulativeDist >= SAMPLING_INTERVAL_METERS) {
                        const fraction = segDist > 0 ? distAlongSeg / segDist : 0;
                        const samplePoint = google.maps.geometry.spherical.interpolate(p1, p2, fraction);
                        const streetCarHeading = google.maps.geometry.spherical.computeHeading(p1,p2);
                        logger.debug(`Sampling at point: ${samplePoint.lat().toFixed(5)},${samplePoint.lng().toFixed(5)} for fetchImagesForStreetSection`);

                        const headingsToTry = [
                            { side: 'R', val: (streetCarHeading + 90 + 360) % 360 },
                            { side: 'L', val: (streetCarHeading - 90 + 360) % 360 }
                        ];

                        for (const headInfo of headingsToTry) {
                            if (!isCrawlingGlobal) {
                                logger.info("fetchImagesForStreetSection: Headings loop interrupted by stop request.");
                                break;
                            }
                            const buildingViewHeading = headInfo.val;
                            logger.debug("New Promise (implicit): getStreetViewDataWithDate in fetchImagesForStreetSection heading loop for side", headInfo.side);
                            const svResult = await getStreetViewDataWithDate(samplePoint, buildingViewHeading); // MODIFIED CALL

                            if (svResult && svResult.imageUrl) {
                                const uniqueImgKey = svResult.panoId + '_' + Math.round(buildingViewHeading);
                                if (!processedPanoHeadings.has(uniqueImgKey)) {
                                    processedPanoHeadings.add(uniqueImgKey);
                                    logger.debug("New Promise (implicit): urlToImageData in fetchImagesForStreetSection heading loop for URL:", svResult.imageUrl.slice(-30));
                                    const imageDataObject = await urlToImageData(svResult.imageUrl);

                                    if (!imageDataObject) {
                                        logger.warn("Failed to get imageData for SV URL in fetchImagesForStreetSection:", svResult.imageUrl);
                                        updateBottomStatusBar({apiKey:API_KEY, apiKeyFullStatus: apiKeyStatusText, processStatus:`Error - Image Load (${svResult.panoId.slice(0,5)})`, progress:null, etrString:""});
                                        continue;
                                    }
                                    logger.debug("New Promise (implicit): reverseGeocodeLatLng in fetchImagesForStreetSection heading loop");
                                    const projectedBuildingLoc = google.maps.geometry.spherical.computeOffset(svResult.carLocation, PROJECTION_DISTANCE_METERS, buildingViewHeading);
                                    const addressInfo = await reverseGeocodeLatLng(projectedBuildingLoc);

                                    collectedForStreet.push({
                                        addressQueryString: addressInfo.fullAddress || `Approx_${projectedBuildingLoc.lat().toFixed(5)}_${projectedBuildingLoc.lng().toFixed(5)}_${Date.now()}`,
                                        houseNumber: addressInfo.houseNumber,
                                        loc:projectedBuildingLoc.toJSON(),
                                        streetViewImageUrl:svResult.imageUrl,
                                        streetViewPanoId:svResult.panoId,
                                        streetViewHeading:buildingViewHeading,
                                        carLocation:svResult.carLocation.toJSON(),
                                        imageCaptureDate: svResult.imageCaptureDate, // <<< ADDED
                                        imageData:imageDataObject
                                    });
                                    logger.debug(`Fetched ${headInfo.side} view (Date: ${svResult.imageCaptureDate}) for ${addressInfo.houseNumber || addressInfo.fullAddress || 'Unknown'} in fetchImagesForStreetSection`);
                                } else {
                                    logger.debug("Skipped already processed pano/heading for side " + headInfo.side + ": " + uniqueImgKey);
                                }
                            } else {
                                logger.debug(`No SV data for side ${headInfo.side} at sample point in fetchImagesForStreetSection.`);
                            }
                        } // End for headingsToTry
                        if (!isCrawlingGlobal) break; // Break from inner while if needed
                        cumulativeDist = 0;
                    } // End if cumulativeDist
                    if (!isCrawlingGlobal) break;
                    const step = Math.min(SAMPLING_INTERVAL_METERS - cumulativeDist, segDist - distAlongSeg);
                    cumulativeDist += step;
                    distAlongSeg += step;
                    if (step <= 0 && segDist > 0) {
                        logger.warn("Segment step was <= 0, breaking in fetchImagesForStreetSection to avoid infinite loop.");
                        break;
                    }
                } // End while distAlongSeg
                if (!isCrawlingGlobal && i < path.length -2 ) {
                     logger.info("fetchImagesForStreetSection: Outer loop interrupted by stop request.");
                     break;
                }
            } // End for path segments
            logger.info(`Finished fetching for street section, collected ${collectedForStreet.length} images.`);
            return collectedForStreet;
        }
        ...
        async function processAndDisplayResultsWithKNN_Centroid(processedTargetImages) {
            logger.info(`Processing ${processedTargetImages.length} target images with Centroid Model & k-NN (v${appVersion}).`);
            currentAnalysisResults = [];
            ...
            logger.debug("New Promise (implicit): getVerifiedFeatures for kNN in processAndDisplayResultsWithKNN_Centroid v"+appVersion);
            ...
            for (const imgResult of processedTargetImages) {
                ...
                try {
                    ...
                    logger.error("Failed to save building record (KNN Centroid v"+appVersion+"):", e, imgResult.addressQueryString);
                }
            ...
            updateSummary();
            logger.info("Display updated with Centroid Model, k-NN, and adaptive results (v"+appVersion+").");
        }

        function redrawResultTables() {
            logger.debug("redrawResultTables v"+appVersion+". Items:", currentAnalysisResults.length);
        ...
        function displayImageResults(resultsList, tableCellElement, scoreLabel) {
            logger.debug("displayImageResults v"+appVersion+" for "+scoreLabel+" Items: "+resultsList.length);
        ...
        function displayUncertainImages(uncertainList) {
            ...
            logger.debug("Displaying uncertain images v"+appVersion+". Count:", uncertainList.length);
        ...
        function updateSummary() {
            logger.debug("updateSummary v"+appVersion+". Items: "+currentAnalysisResults.length);
        ...
        async function handleResultRowDoubleClick(event) {
            logger.debug("New Promise: handleResultRowDoubleClick v"+appVersion);
        ...
        function showConfirmationDialog(message, dialogId = 'tm-general-confirm-dialog-zl-v19final', yesText = "Yes", noText = "No") {
            logger.debug("New Promise: showConfirmationDialog v"+appVersion+" : "+message.slice(0,30));
        ...
        async function applyBatchFeedback(targetStatus) {
            ...
            logger.debug("New Promise: Batch Feedback Confirmation Dialog v"+appVersion);
        ...
        /**
         * Prompts the user to classify a set of candidate images to form a reference set for "renovated" and "unrenovated".
         * Uses a modal UI with image display and classification buttons/keyboard shortcuts.
         * Relies on `allImagesToProcess` global-like array being populated with candidate image metadata
         * including their `imageData` object {data, width, height} before this function is called.
         *
         * @async
         * @param {Array<string>} candidateAddressesForModal - An array of address strings for candidate reference images.
         *                                                 These addresses are used to find corresponding pre-loaded image data
         *                                                 from the `allImagesToProcess` array.
         * @returns {Promise<object|null>} A promise resolving to an object:
         *          { renovated: Array<object>, unrenovated: Array<object> }
         *          where each inner object is { addressQueryString: string, imageData: object {data, width, height} }.
         *          Returns null if the process is cancelled or a critical error occurs.
         *          The collectedReferences object might be empty if the user skips all images.
         */
        async function promptForReferenceSetUI(candidateAddressesForModal) {
            logger.info("New Promise: Prompting user for reference set via swipe UI (v1.9.3). Candidates provided:", candidateAddressesForModal.length);
            const apiKeyStatusText = API_KEY ? (localStorage.getItem(LOCALSTORAGE_API_KEY) === API_KEY ? "Registered (localStorage)" : "Session Only") : "Unregistered";
            updateBottomStatusBar({
                apiKey: API_KEY,
                apiKeyFullStatus: apiKeyStatusText,
                processStatus: "Started - Defining Reference Set",
                progress: null,
                etrString: ""
            });

            const collectedReferences = { renovated: [], unrenovated: [] };
            const overlayId = 'tm-swipe-modal-overlay-zl-v19final-ref'; // Unique ID for this modal instance
            let keydownHandlerForSwipeModalInstance = null; // To store the specific keydown handler

            // Ensure only one modal of this type at a time
            let existingOverlay = document.getElementById(overlayId);
            if (existingOverlay) {
                logger.warn("Swipe modal for reference set (ID:", overlayId, ") is already open. Aborting new prompt.");
                return collectedReferences; // Or null if this is an error state
            }
            ...
            const dialog = document.createElement('div');
            dialog.className = 'dialog-box-zl-ui-v19final';
            dialog.style.minWidth = '420px'; // Adjusted width
            dialog.innerHTML = `
                <h3>Define Reference Buildings (${candidateAddressesForModal.length} candidates)</h3>
                <p>Classify images as Renovated or Unrenovated. Use arrow keys (← Unren., → Renov., ↓ Skip) or buttons.</p>
                <div class="swipe-image-container" id="tm-swipe-image-zl-v19final-ref" style="background-image: url(''); display: flex; align-items: center; justify-content: center; font-style: italic; color: #888; min-height: 240px;">Loading image...</div>
                <p id="tm-swipe-address-zl-v19final-ref" style="text-align:center; font-style:italic; min-height:1.2em; margin-top: 5px;"></p>
                <div class="swipe-actions" style="margin-top:15px; display:flex; justify-content:space-around;">
                    <button id="tm-swipe-unrenovated-zl-v19final-ref" style="background-color:#dc3545; flex-grow:1; margin:0 2px;">${svgIconUnrenovated} Unrenovated</button>
                    <button id="tm-swipe-skip-zl-v19final-ref" style="background-color:#6c757d; flex-grow:1; margin:0 2px;">Skip</button>
                    <button id="tm-swipe-renovated-zl-v19final-ref" style="background-color:#28a745; flex-grow:1; margin:0 2px;">${svgIconRenovated} Renovated</button>
                </div>
                <p id="tm-swipe-progress-zl-v19final-ref" style="text-align:center; margin-top:10px;"></p>
                <button id="tm-swipe-finish-zl-v19final-ref" style="margin-top:15px; background-color:#007bff; display:none;">Finish Defining References</button>
            `;
            overlay.appendChild(dialog);
            document.body.appendChild(overlay);

            const imageContainer = document.getElementById('tm-swipe-image-zl-v19final-ref');
            const addressP = document.getElementById('tm-swipe-address-zl-v19final-ref');
            const progressP = document.getElementById('tm-swipe-progress-zl-v19final-ref');
            const btnUnrenovated = document.getElementById('tm-swipe-unrenovated-zl-v19final-ref');
            const btnSkip = document.getElementById('tm-swipe-skip-zl-v19final-ref');
            const btnRenovated = document.getElementById('tm-swipe-renovated-zl-v19final-ref');
            const btnFinish = document.getElementById('tm-swipe-finish-zl-v19final-ref');

            let currentIndex = 0;
            let currentImageDataForRef = null; // Holds the {data, width, height} for the current image

            // Return a Promise that resolves when the modal interaction is complete
            return new Promise((resolveOuter) => {
                const handleChoice = (choice) => {
                    logger.debug("Reference choice made:", choice, "for index:", currentIndex, "Address:", candidateAddressesForModal[currentIndex]);
                    if (currentIndex < candidateAddressesForModal.length) {
                        const currentAddress = candidateAddressesForModal[currentIndex];
                        if (choice !== 'skip' && currentImageDataForRef) {
                            collectedReferences[choice].push({
                                addressQueryString: currentAddress,
                                imageData: currentImageDataForRef // Store the {data, width, height} object
                            });
                            logger.debug(`Reference set: ${currentAddress} classified as ${choice}. Current count for '${choice}': ${collectedReferences[choice].length}`);
                        } else if (choice !== 'skip' && !currentImageDataForRef) {
                            logger.warn("No imageData available for chosen reference:", currentAddress, "Choice was:", choice, "This image will be skipped.");
                        }
                    }
                    currentIndex++;
                    loadNextCandidate();
                ...
                const closeSwipeModal = (reason) => {
                    logger.info("Closing swipe/reference modal, reason:", reason);
                    if (keydownHandlerForSwipeModalInstance) {
                        document.removeEventListener('keydown', keydownHandlerForSwipeModalInstance);
                        keydownHandlerForSwipeModalInstance = null; // Clear handler
                    }
                    try {
                ...
                async function loadNextCandidate() {
                    logger.debug("loadNextCandidate called, currentIndex:", currentIndex);
                    // Check global crawling flag. Allow first image if part of initial setup, otherwise stop.
                    if (!isCrawlingGlobal && currentIndex > 0 && !dialog.contains(document.activeElement)) {
                        closeSwipeModal("Process stopped externally by isCrawlingGlobal flag");
                        return;
                    }
                    if (currentIndex >= candidateAddressesForModal.length) {
                        addressP.textContent = "All candidates processed!";
                        imageContainer.style.backgroundImage = `url('')`;
                        imageContainer.textContent = "Reference set definition complete!";
                        btnUnrenovated.disabled = true;
                        btnSkip.disabled = true;
                        btnRenovated.disabled = true;
                        btnFinish.style.display = 'none'; // Hide finish button as it's auto-closing

                        logger.info("All reference candidates processed. Auto-closing modal.");
                        // Automatically close after a short delay to let the user see the "Done!" message.
                        setTimeout(() => {
                            closeSwipeModal("All candidates processed");
                        }, 1500); // 1.5 second delay
                        return;
                    }

                    progressP.textContent = `Image ${currentIndex + 1} of ${candidateAddressesForModal.length}`;
                    const address = candidateAddressesForModal[currentIndex];
                    addressP.textContent = address || "Address not specified";
                    imageContainer.style.backgroundImage = `url('')`; // Clear previous image
                    imageContainer.textContent = "Loading image...";
                    currentImageDataForRef = null; // Reset

                    // Find pre-fetched image data from the global `allImagesToProcess` array
                    const preFetchedImage = allImagesToProcess.find(img => img.addressQueryString === address && img.type === 'reference_candidate');

                    if (preFetchedImage && preFetchedImage.imageData) {
                        logger.debug("Found pre-fetched image data for reference candidate:", address);
                        currentImageDataForRef = preFetchedImage.imageData; // This is {data, width, height}
                        const canvas = document.createElement('canvas');
                        ...
                        imageContainer.textContent = ""; // Clear "Loading..."
                    } else {
                        logger.warn("Image data NOT pre-fetched as expected for reference candidate:", address, ". Attempting direct fetch (fallback).");
                        logger.debug("New Promise (implicit): getImageDataForAddress (fallback) in promptForReferenceSetUI for", address);
                        const svImageResult = await getImageDataForAddress(address, `ref_cand_modal_fb_${currentIndex}`);
                        if (svImageResult && svImageResult.imageData) {
                            currentImageDataForRef = svImageResult.imageData;
                            const canvas = document.createElement('canvas');
                            canvas.width = currentImageDataForRef.width;
                            canvas.height = currentImageDataForRef.height;
                            const ctx = canvas.getContext('2d');
                            ...
                            imageContainer.textContent = "Could not load image.";
                            logger.error("Could not load reference candidate image for modal even with fallback:", address);
                            // Auto-skip if load fails critically after a delay to show the message
                            setTimeout(() => {
                                // Check if the modal is still trying to show this image
                                if(imageContainer.textContent === "Could not load image." &&
                                   currentIndex < candidateAddressesForModal.length &&
                                   candidateAddressesForModal[currentIndex] === address) {
                                    handleChoice('skip'); // Auto-skip this problematic one
                                }
                            }, 2500);
                        }
                    }
                ...
                keydownHandlerForSwipeModalInstance = (e) => {
                    if (!document.body.contains(overlay)) { // Modal has been closed, remove listener
                        document.removeEventListener('keydown', keydownHandlerForSwipeModalInstance);
                        keydownHandlerForSwipeModalInstance = null;
                        return;
                    }
                    if (e.key === "Escape") {
                        closeSwipeModal("Escape key pressed");
                    } else if (e.key === "ArrowLeft") {
                        btnUnrenovated.click();
                ...
                document.addEventListener('keydown', keydownHandlerForSwipeModalInstance);

                btnFinish.addEventListener('click', () => closeSwipeModal("User finished defining references"));
                overlay.addEventListener('click', (e) => {
                    if (e.target === overlay) { // Clicked on the overlay background
                        closeSwipeModal("Overlay background clicked");
                    }
                });
                loadNextCandidate();
            });
        ...
        function loadGoogleMapsScript() {
            //alert("load google..");
            logger.debug("New Promise: Load Google Maps API Script (v"+appVersion+").");
            return new Promise((resolve, reject) => {
                if (!API_KEY) {
        ...
        function initializeGoogleMapsServices() {
            logger.info("Google Maps API loaded callback. Initializing services (v+"+appVersion+")...");
            try {
                ...
                logger.info("Google Maps services successfully initialized (v"+appVersion+").");
            } catch (e) {
                logger.error("Error initializing Google Maps Services (v"+appVersion+"): "+e.message);
